...
pub fn get_jobset_evals_paginated(hydra_instance: &HydraInstance, first: u64, last: Option<u64>) -> Vec::<HydraEval> {
    let initial = get_jobset_evals(&hydra_instance.clone()).unwrap();
    let initial_last = initial.last.split("=").last().unwrap();

    let last_page = last.or_else(|| {Some(initial_last.parse::<u64>().unwrap())}).unwrap();

    let mut evals = Vec::<HydraEval>::new();
    for page in first..last_page {
        evals.append(&mut get_wrapper(
            format!(
                "{}/jobset/{}/{}/evals?page={}",
...
hydra_instance.jobset.as_ref().unwrap(),
            ...
            .as_str(),
        ).unwrap().json::<HydraEvalPaginated>().unwrap().evals);
    }
    evals
}
